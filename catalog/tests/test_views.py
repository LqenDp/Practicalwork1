...
class LoanedBookInstancesByUserListViewTest(TestCase):
    ...
    def test_only_borrowed_books_in_list(self):
        ...
        # Check that initially we have only borrowed books in list
        self.assertTrue('bookinstance_list' in response.context)

        # Подсчитываем сколько книг должно быть у testuser1 со статусом 'o'
        expected_count = BookInstance.objects.filter(
            borrower=self.test_user1,
            status='o'
        ).count()

        self.assertEqual(len(response.context['bookinstance_list']), expected_count)
...
class RenewBookInstancesViewTest(TestCase):
    ...
    def setUp(self):
        ...
        catalog_permission = Permission.objects.get(codename='view_book')  # или другие нужные права
        self.test_user2.user_permissions.add(catalog_permission)
        self.test_user2.save()
    ...
    def test_logged_in_with_permission_another_users_borrowed_book(self):
        ...
        self.assertTrue(self.test_user2.has_perm('catalog.can_mark_returned'))
    ...
    def test_form_invalid_renewal_date_past(self):
        ...
        self.assertEqual(resp.status_code, 200)
        # Проверяем, что форма есть в контексте и содержит ошибки
        self.assertTrue('form' in resp.context)
        self.assertTrue(resp.context['form'].errors)
        self.assertIn('renewal_date', resp.context['form'].errors)
        self.assertIn('Invalid date - renewal in past', resp.context['form'].errors['renewal_date'])
    ...
    def test_form_invalid_renewal_date_future(self):
        ...
        self.assertTrue(resp.context['form'].errors)
        self.assertIn('renewal_date', resp.context['form'].errors)
        self.assertIn('Invalid date - renewal more than 4 weeks ahead', resp.context['form'].errors['renewal_date'])
